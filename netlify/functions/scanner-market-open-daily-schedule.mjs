...
export default async function(){
  ...
  if(['Sat','Sun'].includes(weekday)||hour!==9||minute!==30)return new Response('waiting for 09:30 America/New_York',{status:200});
  ...
  try{
    // First refresh the authoritative 100-day split universe, then scrape Finviz
    // Float from that exact universe in the dedicated GitHub worker.
    const universe=await refreshUniverse();
    const worker=await dispatchFloatWorker({source:'daily-market-open'});
    await store.setJSON('scanner-market-open-refresh-date',{date:today,state:'dispatched',startedAt,universeUpdatedAt:universe.updatedAt,worker,updatedAt:new Date().toISOString()});
    await store.setJSON('scanner-market-open-refresh-status',{state:'dispatched',date:today,startedAt,finishedAt:null,error:null,universeUpdatedAt:universe.updatedAt,tickers:universe.tickers?.length||0,worker});
